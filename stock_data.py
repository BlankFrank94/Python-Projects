...
def load_stock_data(stock_list):
    ...
    stockSelectCmd = """SELECT symbol, name, shares FROM stocks; """
    ...
    for row in stockRows:
        ...
        dailyDataCmd = """SELECT date, price, volume FROM dailyData WHERE symbol=?; """
        ...
        for dailyRow in dailyDataRows:
            daily_data = DailyData(dailyRow[0], float(dailyRow[1]), float(dailyRow[2]))
